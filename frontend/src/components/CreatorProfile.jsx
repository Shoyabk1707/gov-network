...
// 🚀 PREMIUM POST CARD COMPONENT (With Read More & Routing)
...
export default function CreatorProfile({ userId, onBack }) {
  ...
  const getTabs = () => {
    ...
    // For Aspirant & Official: Only About and Posts
    return ['About', 'Posts'];
  };
  const tabs = getTabs();
    ...
    <div className="max-w-3xl mx-auto mt-4 space-y-4 pb-12 relative px-4 md:px-0">
          ...
          <div className="pt-20 mt-2">
            <div className="flex flex-wrap items-center gap-3 mb-2">
              ...
              <span className={`px-3 py-1 rounded-full text-xs font-bold text-white ${
                profile.role === 'official' ? 'bg-[#0f172a]' : 
                profile.role === 'creator' ? 'bg-[#ca8a04]' : 'bg-[#059669]'
              }`}>
      ...
      {activeTab === 'About' && (
        <div className="space-y-4 animate-fadeIn">
          ...
          {/* B. Targeted Exams (Aspirant always, Official if exists) */}
          {(profile.role === 'aspirant' || (profile.role === 'official' && profile.targetExams && profile.targetExams.length > 0)) && (
            <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
               ...
               {profile.targetExams && profile.targetExams.length > 0 ? (
                 <div className="flex flex-wrap gap-2">
                   {(Array.isArray(profile.targetExams) ? profile.targetExams : profile.targetExams.split(',')).map((exam, idx) => {
                 ...
                 <p className="text-sm text-slate-500 italic">No targeted exams added yet.</p>
               )}
            </div>
          )}

          {/* C. Experience (For Aspirant & Official) */}
          {profile.role !== 'creator' && profile.experience?.length > 0 && (
            <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
              <h2 className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-5">Experience</h2>
          ...
          {/* D. Education (For Aspirant Only as requested) */}
          {profile.role === 'aspirant' && profile.education?.length > 0 && (
            <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
              <h2 className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-5">Education</h2>
              <div className="space-y-5">
      ...
      {/* 2. ORGANIZATION TAB (Only for Creator/Institute) */}
      {activeTab === 'Organization' && profile.role === 'creator' && (
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6 animate-fadeIn">
          <h2 className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-4">Organization Details</h2>
