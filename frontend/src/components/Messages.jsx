...
export default function Messages() {
  ...
  const getRecipientUser = (chatItem) => chatItem.participants?.find(p => String(p._id || p) !== String(currentUserId)) || {};
                    ...
                    <div key={msg._id || index} className="space-y-3">
                      {showDateDivider && (
                        <div className="flex justify-center my-4 animate-fadeIn">
                          <span className="px-3 py-1 bg-slate-200/70 text-slate-600 rounded-full text-[10px] font-bold uppercase tracking-wider shadow-xs">{currentMsgDate}</span>
                        </div>
                      )}
                      <div className={`flex ${isOwn ? 'justify-end' : 'justify-start'}`}>
                        <div className={`max-w-[75%] px-3.5 py-2 rounded-2xl text-xs font-medium leading-relaxed shadow-xs ${isOwn ? 'bg-slate-900 text-white rounded-br-none' : 'bg-white text-slate-800 border border-gray-150 rounded-bl-none'}`}>
                          <p className="whitespace-pre-wrap">{msg.text}</p>
                          <span className="text-[9px] block text-right mt-1 opacity-60 font-medium">{new Date(msg.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                        </div>
                      </div>
                    </div>
                  );
